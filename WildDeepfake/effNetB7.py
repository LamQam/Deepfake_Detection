...
# Research Configuration for EfficientNetB7
# Reduced from 380x380 for speed while maintaining research validity
IMG_SIZE = (224, 224)
BATCH_SIZE = 32  # Optimized for MacBook memory
EPOCHS = 20  # Reasonable for research
LEARNING_RATE = 1e-4  # Higher than original for faster convergence
FINE_TUNE_EPOCHS = 10
USE_SUBSET = True  # Set to False for full dataset
SUBSET_SIZE = 60000  # Balanced subset for faster experimentation
...
def create_optimized_data_generators():
    """Create data generators optimized for EfficientNetB7 research."""

    # Optimized augmentation - less intensive but research-appropriate
...
def create_efficientnetb7_research_model():
    """Create EfficientNetB7 model optimized for research on deepfakes."""
    ...
    print(f"EfficientNetB7 loaded with input size: {IMG_SIZE}")
    ...
    # Classification layers - simplified for faster training
    x = layers.Dense(512, activation='relu')(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.4)(x)

    x = layers.Dense(128, activation='relu')(x)  # Smaller than original
...
def plot_research_history(history, fine_tune_history=None, save_path='efficientnetb7_research_history.png'):
    """Plot comprehensive training history for research documentation."""
    ...
    axes[0, 0].set_title('EfficientNetB7 - Model Accuracy')
    ...
    if finetune_start:
        axes[0, 1].axvline(x=finetune_start, color='green',
                           linestyle='--', alpha=0.7, label='Fine-tuning starts')
    axes[0, 1].set_title('EfficientNetB7 - Model Loss')
    ...
    if finetune_start:
        axes[1, 0].axvline(x=finetune_start, color='green',
                           linestyle='--', alpha=0.7, label='Fine-tuning starts')
    axes[1, 0].set_title('EfficientNetB7 - AUC Score')
    ...
    if finetune_start:
        axes[1, 1].axvline(x=finetune_start, color='green',
                           linestyle='--', alpha=0.7, label='Fine-tuning starts')
    axes[1, 1].set_title('EfficientNetB7 - Precision & Recall')
...
def evaluate_research_model(model, test_generator):
    """Comprehensive evaluation for research documentation."""
    print("="*60)
    print("EVALUATING EFFICIENTNETB7 ON WILD DEEPFAKE DATASET")
    print("="*60)
    ...
    print(f"\nEFFICIENTNETB7 PERFORMANCE METRICS:")
    ...
    plt.savefig('efficientnetb7_confusion_matrix.png',
...
def main():
    """Research-focused training pipeline for EfficientNetB7."""
    print("🔬 RESEARCH: EfficientNetB7 Transfer Learning on Wild Deepfake Dataset")
    ...
    model, base_model = create_efficientnetb7_research_model()
    ...
    callbacks = [
        ModelCheckpoint(
            'efficientnetb7_wild_deepfake_best.keras',
            monitor='val_auc',
            mode='max',
            save_best_only=True,
            verbose=1
    ...
    print("\n" + "="*50)
    print("PHASE 1: TRAINING WITH FROZEN EFFICIENTNETB7")
    print("="*50)
    ...
    # Phase 2: Fine-tuning
    print("\n" + "="*50)
    print("PHASE 2: FINE-TUNING EFFICIENTNETB7")
    print("="*50)
    ...
    fine_tune_callbacks = [
        ModelCheckpoint(
            'efficientnetb7_wild_deepfake_finetuned.keras',
            monitor='val_auc',
            mode='max',
            save_best_only=True,
            verbose=1
    ...
    plot_research_history(history, fine_tune_history)

    # Final evaluation
    results = evaluate_research_model(model, test_gen)

    # Save final model
    model.save('efficientnetb7_wild_deepfake_final.keras')

    # Research summary
    print("\n" + "="*70)
    print("RESEARCH SUMMARY - EFFICIENTNETB7 ON WILD DEEPFAKE DATASET")
    print("="*70)
